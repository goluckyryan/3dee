...
int main(int argc, char *argv[]){
  ...
  float phiNN = 00;
  float BE    = 4.63;
  output = Knockout2D(14, 8,  245, k, angk, angNN, BE);
 
  printf("k:%6.2f angk:%6.2f angNN:%6.2f  Sp: %6.2f ---> T_c:%9.3f, theta_c:%9.3f, theta_d:%9.3f\n",
         k, angk, angNN, BE,  output[0], output[1], output[3]);    
  printf("T1L:%10.4f, theta_1L:%10.4f, T2L:%10.4f, theta_2L:%10.4f \n", output[4], output[5], output[6], output[7]);

  outputINV = Knockout2Dinv3(14,8, 245, output[0], output[1], output[3],BE);
