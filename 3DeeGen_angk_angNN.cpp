...
int main(int argc, char *argv[]){
  ...
  int totCount = 6*180*180/AngStep/AngStep;

  float *jaco = new float[2]; // jacobian for x-sec transform
  float *output = new float[8]; // knockout output 

  char filename[50];
  sprintf(filename, "paraOut_%2d%s_JA%3.1f_JB%3.1f_k%d_Sp%4.1f.dat",  MA, symbolZ(Z), JA, JB, (int)k, BE);
...
//#############################  display input condition
  ...
  fprintf(paraOut, "\n");
  fprintf(paraOut, "%9s   %9s   %9s   %9s   %9s   %9s   %9s   %9s   %9s   %9s   ",
  ...
  //########################### start looping
  int count = 0;
  for (float angk=1; angk <=180; angk+=AngStep){
    for(float angNN=1; angNN <=180; angNN+=AngStep){
      ...
      fprintf(paraOut,"%9.3f   %9.3f   %9.3f   %9.3f   %9.3f   %9.3f   %9.3f   %9.3f   %9.3f   %9.3f   ",
      ...
      for (int ID=1; ID <=6; ID ++){
        ...
        // save parameters + readout 
        fprintf(paraOut,"%9.3f   %9.3f   ",DWIA ,Ay); 
