...
int main(int argc, char *argv[]){ 
      ...
      for(float angd = angdStart; angd<=angdEnd; angd+=angStep){ 
        for(float phic = phicStart; phic<=phicEnd; phic+=phiStep){ 
          for(float phid = -phidStart-180; phid<=phidEnd-180; phid+=phiStep){ 
            for(float Vso = VsoStart; Vso <= VsoEnd; Vso += VsoStep){ 
  ...
  printf(" Vso step:%4d MeV, Range (%6.1f, %6.1f)\n", VsoStep, VsoStart, VsoEnd);  
  ...
  fprintf(paraOut, "#W%15s%2d MeV, Range (%6.1f, %6.1f)\n", " Vso step =", VsoStep, VsoStart, VsoEnd);  
  int paraNum = 13; 
  ...
  //        "Tc", "theta_c", "phi_c","Td", "theta_d", "phi_d", "beta_d", 
  //        "k", "theta_k", "phi_k", "theta_NN", "phi_NN", 
  //        "T_1","theta_1", "phi_1", "T_2", "theta_2", "phi_2"); 
  fprintf(paraOut, "#%12s%12s%12s%12s%12s%12s%12s%12s%12s%12s%12s%12s%12s",  
          "Tc", "theta_c", "phi_c","Td", "theta_d", "phi_d", "beta_d", 
          "k", "theta_k", "phi_k", "theta_NN", "phi_NN", "Vso"); 
        ...
        for(float phic = phicStart; phic<=phicEnd; phic+=phiStep){ 
          for(float phid = phidStart-180; phid<=phidEnd-180; phid+=phiStep){ 
            for(float Vso = VsoStart; Vso <= VsoEnd; Vso += VsoStep){ 
              ...
              printf("        T1:%9.3f, theta_1:%9.3f, phi_1:%9.3f, T2:%9.3f, theta_2:%9.3f, phi_2:%9.3f, Vso:%9.3f\n", 
                     output[0], output[1], output[2], output[3], output[4], output[5], Vso); 
          
              // save parameters + readout 
              fprintf(paraOut," %12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f%12.3f", 
                      Tc, angc, phic, output[7], angd, phid2,  betad, 
                      output[8], output[9], output[10], output[11], output[12],Vso);
              //         output[0], output[1], output[2], output[3], output[4], output[5], Vso);        
