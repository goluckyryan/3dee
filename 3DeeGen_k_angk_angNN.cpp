...
int main(int argc, char *argv[]){
  ...
  int totCount = 6*300*360*100/kStep/angStep/angNNStep;
...
//#############################  display input condition
  ...
  for (float k=0; k <300; k+=kStep){
    for (float angk = 0; angk<360; angk+=angStep){
      for(float angNN = 40; angNN<140; angNN+=angNNStep){
        ...
        printf("                    beta:%4.2f jaco:%10.7f , angleA:%10.7f ", output[8], jaco[0], 180-jaco[1]);
        if (jaco[0] < 0 ) {
          count += 6;
          printf(" -- rejected! \n");
          continue;
